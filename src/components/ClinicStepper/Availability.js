...
const Availability = ({ setActiveStep }) => {
  ...
  const callslots = () => {
    fetch(
      `https://reservefree-backend.herokuapp.com/get/slots?clinicId=${clicicId}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const slots = { clinicId: clicicId, slot: appointment };
    console.log(slots);
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      >
        ...
        >
          <button className="stepper_btn" onClick={finishStep}>
            Finish
          </button>
        </Box>
      </div>
